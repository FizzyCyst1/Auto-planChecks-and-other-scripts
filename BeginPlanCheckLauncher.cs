    ...
    public class Script
    {
        ...
        public void Execute(ScriptContext context)
        {
            try
            {
                ...
                string launcherPath = @"\\Ordcari-mfs001\va_data$\PublishedScripts\StandAlone\BeginPhysicsPlanCheck\BeginPlanCheck\bin\release";
                /**** remove domain name from user ID ****/
                string currentUser = context.CurrentUser.Id.Trim().Replace(@"nswhealth\", "");
                string un = context.CurrentUser.Name;
                /**** Set the exe to run script ****/
                string esapiStandaloneExecutable = @"BeginPlanCheck_v1.exe";
                ...
                string arguments = context.PlanSetup == null
                                    ...
                                    : string.Format("{0};{1};{2};{3};{4};{5}", context.Patient.Id, context.Course.Id, context.PlanSetup.Id, context.StructureSet.Id, currentUser, context.CurrentUser.Name);
